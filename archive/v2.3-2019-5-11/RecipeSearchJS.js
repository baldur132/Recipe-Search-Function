...
let result, rsString, rsParsed, query, i, tableData, loading, metaInfo,
    currentState, searchErrorText, parameters, textFieldValue;
let count = 0;
let columnID = 0;
let recipeDataArray = [];
let error = false;
...
function displayResults(){
    ...
    let tableData = '';
    let count = recipeDataArray.length;
    for (i = 0; i < count; i++){
        tableData += generateTable(recipeDataArray[i]);
    }
    document.getElementById('recipeData').innerHTML = tableData;
    updateSortDisplay();
    setLoadingAnimation(false);
}
