...
function searchFunction($userQuery, $orderData, $parameters){
    ...
    if($tableData[0]['errorMsg'] === '404' && $forceStrict !== 'true'){
        $querySet = parseQuerySoft($userQuery, $orderData);
...
function parseQuerySoft($stringInput, $orderData){
    ...
    $querySet[0] = $queryHead." RecipeTitle LIKE '%".$stringInput."%'".$dataOrder;
    ...
    for($i = 0; $i < $queryCount; $i++){
        $querySet[$i + 1] = $queryHead." RecipeTitle LIKE '%".$queries[$i]."%'".$dataOrder;
    }
    return $querySet;
}
