...
function readURLnUpdate() {
    ...
    if (hash) {
        ...
        textFieldValue.value = hash.replace(/(%20)/g, ' ');
        initiateSearch();
    }
}
...
function displayResults(){
    ...
    //check bad sql generation
    if (metaInfo.errorMsg === '500') {
        console.log('bad sql detected');
        document.getElementById('displayError').innerHTML = '';
        errorText += '500 - Bad SQL Statement Generated<br>';
        document.getElementById('searchData').display = 'none';
        changeDisplayStyle(2);
        return;
    }
    ...
    if (Array.isArray(metaInfo.colErr) && metaInfo.colErr[0] !== undefined) {
        ...
        searchErrorText += '--> Replacing Instances with "RecipeTitle"';
    }
    //check bad user order
    if (chkbx2.checked === true) {
        if (metaInfo.orderError !== 'none') {
            console.log('bad user order detected');
            searchErrorText += '<br>Invalid Order Data - Please Check Syntax or Uncheck "Carry Order Data" Checkbox';
            showErrors = true;
        }
    }
    //display errors
    if(error === false && showErrors === true){
