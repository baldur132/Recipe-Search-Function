...
function searchFunction($userQuery, $orderData, $parameters){
    ...
    $inputEscaped = escapeValidateInput($userQuery, '');

    $parseData = parseQueryStrict($inputEscaped, $orderData);
    $query0 = $parseData[0];
    if($forceExact === 'true'){
    ...
    if(escapeValidateInput($query0, 'v')) {
        ...
        $resultData[0]['errorMsg'] = '500';
        $resultData[0]['errorDetails'] .= 'Bad SQL Statement;';
        $resultData[1] = array('errorMsg' => '404');
    }
...
function parseQuerySoft($stringInput, $orderData){
    ...
    if(preg_match('#(,|:|and|or)#i', $stringInput) === 1) {
        if(preg_match('#(and)#i', $stringInput)){
            $stringInput = preg_replace('#(and)#i', ' ', $stringInput);
        }
        if(preg_match('#(or)#i', $stringInput)){
...
function escapeValidateInput($input, $task){
    //Connect to database
    $database = '';
    $username = '';
    $password = '';
