...
function displayResults() {
    ...
    for (i = 0; i < searchCount; i++) {
        if (Number(recipeDataArray[i][0].numResults) === 0) {emptyCount++;}
    }
    ...
    if (allEmpty === false) {
        ...
        for (e = 0; e < searchCount; e++) {
            resultNum = Number(recipeDataArray[e][0].numResults) + resultNum;
            if (!(Number(recipeDataArray[e][0].numResults) <= 0)) {
                if (e !== 0) {
                    tablePost += '<tr><td colspan="9" class="tableDivider">Search for "' + recipeDataArray[e][0].content + '"</td>';
                }
